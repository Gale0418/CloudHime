...
The release verifier computes every manifest SHA-256 with a 1 MiB sequential stream, so it keeps full integrity coverage without loading runtime binaries into memory. The local real-dist preflight test has a bounded 1200-second default timeout for slow disks; a timeout is an incomplete verification, never a pass.
