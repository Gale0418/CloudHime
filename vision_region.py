...
def build_region_vision_prompt(
    ...
    *,
    ...
    lines = [
        ...
        "Return exactly one region object for each supplied region hint, using each supplied id exactly once.",
        "Never invent ids or return a region for a hint that was not supplied.",
        "If exactly one full-image hint is supplied, return exactly one region using that hint id; do not split it into new ids.",
        "The translation value must be written in the requested target language, not English unless the target language is English.",
