...
- 主人後續確認同時開著 Steam；host 也觀察到 Steam 程序。CloudHime R4 package／source 均無 `gldriverquery.exe` 或 `SDL2.dll`，因此該彈窗保留為 Steam 側事件，不列為 CloudHime release blocker；若未開 Steam 仍可重現才重新歸因。
- 2026-09-21 R4 development self-sign install gate PASS：只複製 unsigned MSIX 到隔離 stage，以短效 `CN=CloudHime Development` 憑證簽署副本，`signtool verify /pa /all` 通過；安裝並以 AUMID `CloudHime_4nvnqyjwyamgj!CloudHime` 啟動 5 秒，隨後卸載。原始 MSIX SHA256 在前後均為 `4ac4e4f9e0e3d5f51d5abf423b051eb15940b79b087d545a347a0a27411bc29f`；package、CurrentUser／LocalMachine 憑證、stage 與 CloudHime-owned process 殘留均為 0。原始 MSIX 仍未簽章；此 gate 不冒充 Store 公開信任。
- 2026-09-21 R4 WACK 完整執行 PASS：官方 `appcert` 對 `dist/offline-20260920-r4/msix/CloudHime-0.1.0.0-x64.msix` 完成測試，XML `OVERALL_RESULT=PASS`、`PARTIAL_RUN=FALSE`，2,478,034 bytes，SHA256 `D23A6395C2A6E6B6746BF92BF9981556340E86B208E650D7FB8AC470FE0D1188`；報告位於 `output/mission-center-evidence/ch-t64-r4-wack-20260921.xml`。結束後 `appcert`／`aitstatic` 與 CloudHime package 殘留均為 0。WACK 是本機送審前品質 gate，不等於 Partner Center certification。
- T64 仍維持 Review：最新 R4 的 Sandbox 純 CPU 圖片功能、development install／launch／uninstall 與 WACK 都已通過；尚缺正常 UI 中「Use Local Gemma」不回彈 Google 的主人確認。Partner Center 帳號已由 T57 完成，但 CloudHime 產品身分尚未建立，後續由 T53～T55 處理。
- 2026-09-21 R7 clean Windows Sandbox 主人驗收 PASS：完整離線包在無 Key 環境啟動，Gemma AI 可選入隨包 Local Gemma 3 4B 且不再回彈 Google；設定頁顯示 `Ready - Local AI - Gemma 3 4B (Local)`。這解掉 R4 遺留的正常 UI owner-confirmation gate。
- 依主人角色設定，以內建 ImageGen 產生新的白天／夜晚天空宮殿偵探公主背景；來源 PNG 取代舊 JPG，未納入範例圖片或模型到 GitHub。Impeccable 檢查後將上方控制區改成 light/dark 224/255 霧面透明，高對比仍保持不透明；UI regression `80 passed`，CodeRabbit 最終審查 `0 findings`。
- R8 最終 frozen build／light ZIP／full dist 完成：full dist 368 files、2 models、4,862,936,315 bytes；release provenance 與模型 exact size／SHA gate PASS。發行／MSIX contracts `57 passed, 1 deselected`；deselected 項為會重複 hash 真實 4.86GB dist 的 optional preflight，實際 verifier 已由 build 流程通過。
- R8 unsigned MSIX `dist/offline-20260921-r8/msix/CloudHime-0.1.0.0-x64.msix` 為 3,920,876,614 bytes，SHA256 `1D7832AD51CE10AF70BF29B10F226D3F65EBE859C9C2DEA77542B7B398B2027C`。短效開發簽章只套用隔離副本；SignTool verify、安裝、AUMID 啟動 5 秒與卸載 PASS，原件 hash 前後一致，package／CurrentUser cert／LocalMachine cert／stage 殘留均為 0。
- R8 WACK 完整 PASS：XML `OVERALL_RESULT=PASS`、`PARTIAL_RUN=FALSE`，2,478,034 bytes，SHA256 `3993C3DF24F8FFCF9C0BDE6CBD620F8E65CFCC658052147CDC7FB32E150B2C0C`；`appcert`／`aitstatic`／package 殘留均為 0。R8 frozen EXE 另做隔離環境 20 秒 launch／cleanup PASS。
- T64 本機發行供應鏈與 clean-machine acceptance 已完成；Partner Center 產品身分、Store identity、正式 submission／certification 仍由 T53～T55 獨立追蹤，不冒充公開信任簽章或 Store 上架完成。
