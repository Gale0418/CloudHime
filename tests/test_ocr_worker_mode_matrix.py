...
def test_fullscreen_local_vision_failure_admits_strict_crop_batch_by_default(monkeypatch, qtbot):
    image = np.zeros((120, 240, 3), dtype=np.uint8)
    provider = SimpleNamespace(
        available=lambda: True,
        translate_screenshot=Mock(side_effect=RuntimeError("direct vision failed")),
...
def test_fullscreen_crop_batch_admission_has_safe_opt_out(monkeypatch, qtbot):
    worker = OCRWorker()
    try:
        monkeypatch.delenv(workers_module.FULLSCREEN_CROP_BATCH_ADMISSION_ENV, raising=False)
        assert worker.fullscreen_crop_batch_admission_mode_enabled() is True
        monkeypatch.setenv(workers_module.FULLSCREEN_CROP_BATCH_ADMISSION_ENV, "off")
        assert worker.fullscreen_crop_batch_admission_mode_enabled() is False
    finally:
        worker.cleanup()

def test_fullscreen_crop_batch_rejects_duplicate_region_ids(
    monkeypatch, qtbot
):
    image = np.zeros((120, 240, 3), dtype=np.uint8)
    provider = SimpleNamespace(
        ...
        translate_screenshot=Mock(side_effect=RuntimeError("direct vision failed")),
        interpret_regions=Mock(return_value=[
            SimpleNamespace(id=0, source_text="duplicate-a", translation="duplicate-a"),
            SimpleNamespace(id=0, source_text="duplicate-b", translation="duplicate-b"),
        ]),
    )
    worker = OCRWorker()
    _configure_text_worker(worker, image)
    ...
    try:
        ...
        assert finished == [[['text-route', 17, 21, 40, 16]]]
        assert provider.translate_screenshot.call_count == 1
        provider.interpret_regions.assert_called_once()
        assert not any(
            event.detail == "translation_fullscreen_crop_batch_vision_completed"
...
def test_fullscreen_local_vision_ocr_rescue_retries_once_with_hint_on_degenerate_response(
    monkeypatch, qtbot
):
    image = np.zeros((80, 160, 3), dtype=np.uint8)
    provider = SimpleNamespace(
        ...
        transcribe_screenshot=Mock(
            side_effect=[
                ValueError("degenerate_local_multimodal_ocr_response"),
                TranslationResult(
                    text="守りに特化で",
                    provider="local_multimodal",
                    model="gemma-3-4b-it",
    ...
    worker.build_screenshot_text_hint = Mock(return_value="hint")
    worker.translate_screenshot_gemma = Mock(
        side_effect=ValueError("empty_local_multimodal_screenshot_response")
    )
    worker.run_ocr_with_best_threshold = Mock(
        side_effect=AssertionError("bounded Vision OCR retry must not use Windows OCR")
    )
    finished = []
    worker.finished.connect(finished.append)
    monkeypatch.setattr(workers_module.time, "sleep", lambda _seconds: None)
    ...
    try:
        ...
        assert finished == [[['專精於防守', 7, 11, 160, 80]]]
        assert provider.transcribe_screenshot.call_args_list[0].args == (
            [{"inline_data": {"data": "vision"}}],
        )
        assert provider.transcribe_screenshot.call_args_list[1].kwargs == {
            "source_text_hint": "hint",
        }
        worker.build_screenshot_text_hint.assert_called_once_with(image)
        provider.translate.assert_called_once_with(
            "守りに特化で",
            target_lang=worker.translation_target_lang,
        )
